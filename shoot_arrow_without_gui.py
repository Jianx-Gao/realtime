...
while(True):
    ...
    for p in range(len(predictions)):
        ...
        for i in range(len(points)):
            ...
            # cv2.putText(image, "{}".format(i),(int(points[i][0]), int(points[i][1])),cv2.FONT_HERSHEY_SIMPLEX,1, (0,0,255), 1)
            if i==12:
                if 6 in exist_point:
                    cv2.line(image, (int(points[6][0]), int(points[6][1])), (int(points[12][0]), int(points[12][1])),
                             color[int(6)], 3)
                continue
            if i==11:
                if 5 in exist_point:
                    cv2.line(image, (int(points[5][0]), int(points[5][1])), (int(points[11][0]), int(points[11][1])),
                             color[int(5)], 3)
                continue

            if i-2 in exist_point:
                cv2.line(image, (int(points[i-2][0]), int(points[i-2][1])), (int(points[i][0]), int(points[i][1])), color[int(i/2)],3)
        if 0 in exist_point and 1 in exist_point:
            cv2.line(image, (int(points[0][0]), int(points[0][1])), (int(points[1][0]), int(points[1][1])),color[int(1)],3)
        # Todo: 头
        if points[0][0]!=0:
            cv2.ellipse(n_image,(int(points[0][0]), int(points[0][1])+10), (40,50),0,0,360,(255,255,255),-1)
        # Todo: 脖子
        if points[5][0] != 0:
            cv2.rectangle(n_image,(int(points[0][0])-10, int(points[0][1])+30),(int(points[0][0])+10, int(points[5][1])),(255,255,255),-1)
        # Todo: 身体
        if points[5][0] != 0 and points[12][0] != 0:
            cv2.rectangle(n_image,(int(points[5][0]), int(points[5][1])),(int(points[12][0]), int(points[12][1])),(255,255,255),-1)
        # Todo: 右手
        if points[9][0] != 0and points[5][0] != 0:
            cv2.line(n_image,(int(points[7][0]), int(points[7][1])), (int(points[5][0]), int(points[5][1])),(255,255,255),5)
        if points[9][0] != 0and points[7][0] != 0:
            cv2.line(n_image,(int(points[7][0]), int(points[7][1])), (int(points[9][0]), int(points[9][1])),(255,255,255),5)
        # Todo: 左手
        if points[8][0] != 0and points[6][0] != 0:
            cv2.line(n_image,(int(points[8][0]), int(points[8][1])), (int(points[6][0]), int(points[6][1])),(255,255,255),5)
        if points[10][0] != 0and points[8][0] != 0:
            cv2.line(n_image,(int(points[8][0]), int(points[8][1])), (int(points[10][0]), int(points[10][1])),(255,255,255),5)
        # Todo: 左腿
        if points[12][0] != 0and points[14][0] != 0:
            cv2.line(n_image,(int(points[12][0]), int(points[12][1])), (int(points[14][0]), int(points[14][1])),(255,255,255),5)
        if points[14][0] != 0and points[16][0] != 0:
            cv2.line(n_image,(int(points[14][0]), int(points[14][1])), (int(points[16][0]), int(points[16][1])),(255,255,255),5)
        # Todo: 右腿
        if points[11][0] != 0 and points[13][0] != 0:
            cv2.line(n_image, (int(points[11][0]), int(points[11][1])), (int(points[13][0]), int(points[13][1])),
                     (255, 255, 255), 5)
        if points[13][0] != 0 and points[15][0] != 0:
            cv2.line(n_image, (int(points[13][0]), int(points[13][1])), (int(points[15][0]), int(points[15][1])),
                     (255, 255, 255), 5)
        # Todo: 左开弓
    ...
    if arrow_flag==1:
        cv2.line(n_image,(arrow_x,arrow_y),(int(arrow_x+35*math.cos(arrow_angle)),int(arrow_y+35*math.sin(arrow_angle))),(255, 255, 255), 2)
...
# cap = cv2.VideoCapture("video/1601428211768.mp4")  # 打开视频
# while cap.isOpened():
